...
//		음식을 받기 원하는 시각
		...
		System.out.printf("짜장면: %d시 %d분",now.get(Calendar.HOUR_OF_DAY),now.get(Calendar.MINUTE));
		
		now.set(Calendar.HOUR_OF_DAY, hour);
		now.set(Calendar.MINUTE, minute);
		now.add(Calendar.MINUTE,-18);
		System.out.printf("치킨: %d시 %d분",now.get(Calendar.HOUR_OF_DAY),now.get(Calendar.MINUTE));
