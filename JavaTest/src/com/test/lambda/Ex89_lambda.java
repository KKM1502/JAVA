	...
	private static void m2() {
		...
		try {
			BufferedReader reader
				= new BufferedReader(new FileReader("C:\\class\\code\\java\\JavaTest\\dat\\user.txt"));
			String line = null;
...
//			
			...
			for (User user : list) {
				System.out.println(user.getAge() + "살 "+ user.getName()+"("+user.getGender()+")" +"("+user.getCity()+ ")" );
			}
	...
	private static void m1() {
		...
		/*
		 	...
		 	자바의 정수 리터럴
		 	- 자바는 어떤 기수법으로 데이터를 작성하던, 출력할때는 10진수로 출력한다.
...
class User { 
	...
	User(String no, String name, Integer age, String gender, String city){
		this.no = no;
		this.name = name;
		this.age = age;
		this.gender = gender;
